...
public class PrepareAllToDeployAction extends PrepareToDeployAction {
	...
	public void update(AnActionEvent e) {
		...
		final Project project = e.getData(CommonDataKeys.PROJECT);
		if (project != null) {
			moduleCount = Arrays.stream(ModuleManager.getInstance(project).getModules()).filter(PluginModuleType::isOfType).limit(2).count();
		}
		...
		if (enabled) {
			e.getPresentation().setText(DevKitBundle.message("prepare.for.deployment.all") + " and Install");
		}
	}
}