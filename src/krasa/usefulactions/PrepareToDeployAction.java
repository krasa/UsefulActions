...
public class PrepareToDeployAction extends AnAction {
			...
			public void finished(final boolean aborted, final int errors, final int warnings, final CompileContext compileContext) {
				...
				ApplicationManager.getApplication().invokeLater(() -> {
					...
					if (!errorMessages.isEmpty()) {
						...
						final String title = pluginModules.size() == 1 ? DevKitBundle.message("success.deployment.message", pluginModules.get(0).getName())
						...
						SwingUtilities.invokeLater(() -> PluginManagerConfigurable.shutdownOrRestartApp());
	...
	public static boolean doPrepare(final Module module, final List<String> errorMessages, final List<String> successMessages) {
		...
		if (oldFile.exists()) {
			if (Messages.showYesNoDialog(module.getProject(), DevKitBundle.message("suggest.to.delete", oldPath), DevKitBundle.message("info.message"),
	...
	private static void processLibrariesAndJpsPlugins(final File jarFile, final File zipFile, final String pluginName, final Set<Library> libs,
	...
	private static void makeAndAddLibraryJar(final VirtualFile virtualFile, final File zipFile, final String pluginName, final ZipOutputStream zos,
			final Set<String> usedJarNames, final ProgressIndicator progressIndicator, final String preferredName) throws IOException {
		...
		final String jarName = getLibraryJarName(virtualFile.getName() + JAR_EXTENSION, usedJarNames,
	...
	private static void addLibraryJar(final VirtualFile virtualFile, final File zipFile, final String pluginName, final ZipOutputStream zos,
			final Set<String> usedJarNames, final ProgressIndicator progressIndicator) throws IOException {
	...
	public void update(AnActionEvent e) {
		...
		if (enabled) {
			e.getPresentation().setText(DevKitBundle.message("prepare.for.deployment", module.getName()) + " and Install");
		}
	}

	private static boolean install(@NotNull File file) {
		try {
